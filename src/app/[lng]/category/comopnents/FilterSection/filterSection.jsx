...
const FilterSectionDesktop = ({ setSelectedOptionsHead , responseData , paramsData , setParamsData }) => {
    ...
    const singleSelect = [{ cat: {name: isArabic ? "تصنيف" : "sort", value:"sort_by"}, options: [{name:isArabic ? "المنتجات الجديدة " : "New Arivals",value:"new_arrivals"},{name: isArabic ? "السعر: من الأقل للأعلى" : "Price Low to high",value:"price_low_to_high"},{name:isArabic ? "السعر: من الأعلى للأقل" : "Price High to Low",value:"price_high_to_low"}] }]
    const multiSelect = superCollection

    const stockSelect = [{cat: {name:isArabic ? "متوفر" : "AVAILABLE", value:"inStock"}, options: [{name: isArabic ? "في المخزون" : "In Stock",value:"inStock"}]}]
                        ...
                        <div className={style.filterSectionCategory}>
                            ...
                            {isCollapse && <div className={style.options}>
                                {category && category.length > 0 ? category.map((ele) => {
                                    const { id = "", collectionName = "",collectionNameArabic } = ele || {};
                                    ...
                                    return (
                                        <div className={style.optionsTxt} onClick={() => onClickSelection(collectionName)} >
                                            <div className={style.tickBox}><img src={image} alt="check box" /></div>
                                            <div className={style.elements}>{isArabic ? collectionNameArabic : collectionName}</div>
                                        </div>
                                    )
                                }) : <></>}
                            </div>}
...
const FilterSectionMobile = ({ setSelectedOptionsHead, responseData, slectedFilter, setSelectedFilter , paramsData , setParamsData}) => {
    ...
    const singleSelect = [{name:isArabic ? "المنتجات الجديدة " :"New Arivals",value:"new_arrivals"},{name:isArabic ? "السعر: من الأقل للأعلى" : "Price Low to high",value:"price_low_to_high"},{name:isArabic ? "السعر: من الأعلى للأقل" : "Price High to Low",value:"price_high_to_low"}]
    // const singleSelect = [1, 2, 3, 4]
    const stockSelect = [{cat:isArabic ? "متوفر" : "AVAILABLE", value:"inStock", options: [{name:isArabic ? "في المخزون" : "In Stock",value:"inStock"}]}]
                    ...
                    <div className={style.mobileCategories} >
                        ...
                        {selectedStockFilter.map((item) => {
                            ...
                            {selectedCatogries.map((item) => {
                                const { id = "", collectionName = "",collectionNameArabic } = item || {};
                                let isOptionSelected = false
