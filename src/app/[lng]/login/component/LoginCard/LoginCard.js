...
const validateForm = (formData) => {
  ...
  if(!formData.userEmail){
    errors.userEmail = formData.isArabic ? "البريد الإلكتروني مطلوب" : "Email is required";
  }else if (!/\S+@\S+\.\S+/.test(formData.userEmail)) {
    errors.userEmail = formData.isArabic ? "يرجى إدخال بريد إلكتروني صالح" :  'Invalid email address.';
  }
  if(!formData.password){
    errors.password = formData.isArabic ? "كلمة المرور مطلوبة" : "Password is required";
  }else if(!(formData.password.length > 7)){
    errors.password = formData.isArabic ? "يجب أن تكون كلمة المرور على الأقل 8 أحرف" :  "Passwords need to be a min. of 8 characters";
  }
  return errors;
};
...
export default function Login() {
    ...
    const onLogin = async() =>{
      const validationErrors = validateForm({userEmail:userEmail,password:password,isArabic:isArabic });
      ...
      return (
        <>
         <div className={styles.loginCardWrapper}>
          <div className={styles.loginTxt}>{isArabic ? "تسجيل الدخول" : "Login"}</div>
          ...
          <div className={styles.loginInputContainer}>
              <div>
                  <input autocomplete="off" autoComplete='off' className={`${styles.inputBox} ${isArabic ? styles['inputBox-ar'] : styles['inputBox-en']}` } type='email' onChange={onEmailChange} value={userEmail} placeholder={isArabic ? "البريد الإلكتروني (مثال: abc@gmail.com)" : 'Email ID (ex. abc@gmail.com)'} />
                  {errors.userEmail && <span className={styles.errorMsg}>{errors.userEmail}</span>}
              </div>
              <div className={styles.loginPasswordInput}>
                  <input autocomplete="off" autoComplete='off' className={`${styles.inputBox} ${isArabic ? styles['inputBox-ar'] : styles['inputBox-en']}` } type={isPasswordVisible ? 'text' : 'password'} onChange={onPasswordChange} value={password} placeholder={isArabic ? "كلمة المرور" : 'Password' }
                  />
                   <img
              ...
              />
                  ...
                  <div className={styles.forgetPassword} onClick={()=>router.push('/forget-password')}>{isArabic ? "نسيت كلمة المرور" : "Forgot Password"} ?</div>
              </div>
