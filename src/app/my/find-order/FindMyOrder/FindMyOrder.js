...
//    if (res.map((data) => data.id).includes(searchOrderId)) {
//     console.log("res", res);
//     setOrderList(res);
//     setIsLoading(false);
...
//    }
//     else{
//       setIderror("please enter your valid id")
//     }
//   }
...
//         </div>
//     </div>
//     <div className={styles.idError}>{error}</div>
//     <div>{iderror}</div>
//     {isLoading ? (
    ...
    if (!searchOrderId) {
      ...
      try {
        const response = await fetch(
          `https://api.kuwa.bevaleo.dev/module/find-my-order/${searchOrderId}`
        );
