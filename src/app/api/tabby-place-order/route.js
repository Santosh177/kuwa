...
export async function POST(request,res) {
  ...
  const endpointURL = productId ? `${process.env.BACKEND_END_POINT_URL}/api/v2/tabby/place-order` : `${process.env.BACKEND_END_POINT_URL}/api/v1/tabby/place-order`;
    const placeOrder = await fetch(endpointURL, {
      ...
      console.log("tabbyPlaceOrderResponse",placeOrderResp)
